...
class SaveJsonParserTest {
    ...
    fun `given empty save when to json then return valid json`() {
        ...
        val flags = Flags()
        flags["Test"] = true
        val save = Save("Test", flags.toMap(), StoryPosition(0, 1, 2))
        val expected = """{"name":"Test","flags":{"Test":true},"position":{"chapter":0,"scene":1,"step":2}}"""
    ...
    fun `given valid json when from json then return valid save`() {
        // Given
        val json = """{"name":"Test","flags":{"Test":true},"position":{"chapter":0,"scene":1,"step":2}}"""
